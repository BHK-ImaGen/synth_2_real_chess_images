#!/usr/bin/env python3
# Ablation 02: remove L1 reconstruction loss entirely (GAN + VGG only)
# - Uses BCE adversarial loss (like run10)
# - Weak D (base_channels default 32)
# - Keeps VGG perceptual loss
# - Still supports R1
# - Saves checkpoints and runs inference on test_synth_dir every save_every epochs
...
class DatasetRootFENDataset(Dataset):
    ...
    def __init__(
        ...
        if verbose:
            print(f"[DatasetRoot] Loaded {len(self.items)} usable FEN folders (excluded_present={excluded_present})")
    ...
    def __getitem__(self, idx):
        ...
        if synth is None:
            raise ValueError(f"Could not read synth: {info['synth_path']}")
        if real is None:
            raise ValueError(f"Could not read real: {real_path}")
        return synth, real
...
class PatchDiscriminator(nn.Module):
    def __init__(self, base_channels=32, n_layers=4, use_norm=True):
        super().__init__()
        ...
        layers = []
        layers += block(6, base_channels, stride=2, norm=False)
        ch = base_channels
...
@torch.no_grad()
def run_test_folder_inference(gen, test_synth_dir: Path, out_dir: Path, device, image_size: int, syn_norm_mode: str):
    gen.eval()
    ensure_dir(out_dir)

    if not test_synth_dir.exists():
        print(f"[TestViz] Missing dir: {test_synth_dir}")
        return
    ...
    print(f"[TestViz] Inference on {len(img_paths)} images: {test_synth_dir}")
    for p in img_paths:
...
def save_run_config(logs_dir: Path, config: dict):
    ...
    print(f"[Run] Saved config: {logs_dir / 'config.json'}")
...
def main():
    ...
    ap.add_argument("--syn_norm", default="tanh", choices=["tanh", "center", "none", "imagenet"])
    ap.add_argument("--real_norm", default="tanh", choices=["tanh", "center", "none", "imagenet"])
    ...
    base = DatasetRootFENDataset(dataset_root=args.dataset_root, verbose=True)
    ds = ProcessedDatasetRoot(base, size=args.image_size)
    dl = DataLoader(ds, batch_size=args.batch_size, shuffle=True,
                    num_workers=args.num_workers, pin_memory=True, drop_last=True)

    gen = UNetGenerator().to(device)
    disc = PatchDiscriminator(base_channels=args.disc_base_channels, n_layers=args.disc_layers, use_norm=True).to(device)
    ...
    print(f"[Run] All outputs: {run_root}")

    config = {
        "ablation": "NO_L1",
        "note": "L1 disabled (lambda=0). Uses BCE adversarial + VGG perceptual + R1.",
        "args": vars(args),
        "effective_l1_lambda": 0.0,
    }
    ...
    for epoch in range(args.epochs):
        ...
        for syn01, real01 in dl:
            ...
            # R1
            if args.r1_gamma and args.r1_gamma > 0:
                ...
                r1 = grad_real.pow(2).reshape(grad_real.shape[0], -1).sum(dim=1).mean()
            ...
            g_loss = g_adv + (args.vgg_lambda * g_perc)
        ...
        print(f"[Epoch {epoch+1}/{args.epochs}] Train: G {g_sum/max(1,nb):.4f} | D {d_sum/max(1,nb):.4f}")
        ...
        if (epoch + 1) % args.save_every == 0:
            ...
            print(f"[CKPT] Saved: {gen_path} and {disc_path}")
    ...
    torch.save(disc.state_dict(), ckpt_dir / "disc_final.pth")
    print("[Done] Final checkpoints saved.")
