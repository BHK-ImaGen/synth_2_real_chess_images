...
class UNetGenerator(nn.Module):
    ...
    def forward(self, x):
        s1 = self.d1(x); s2 = self.d2(s1); s3 = self.d3(s2); s4 = self.d4(s3); s5 = self.d5(s4)
        b = self.b(s5)
        x = self.u1(b, s5); x = self.u2(x, s4); x = self.u3(x, s3); x = self.u4(x, s2); x = self.u5(x, s1)
        return self.final(x)
...
def main():
    ...
    config = {
        "ablation": "NO_VGG",
        "note": "VGG removed entirely. Keeps GAN(BCE)+L1+R1. Weak D width32 like run10.",
        "args": vars(args),
    }
    ...
    for epoch in range(args.epochs):
        ...
        for syn01, real01 in dl:
            ...
            g_loss = g_adv + (args.l1_lambda * g_l1)
        ...
        if (epoch + 1) % args.save_every == 0:
            ...
            print(f"[CKPT] Saved {gen_path.name}, {disc_path.name}")
